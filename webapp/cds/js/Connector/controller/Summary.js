 ...
 */
Ext4.define('Connector.controller.Summary', {
    ...
    init : function() {
        ...
        this.control('groupsave', {
            groupsaved : function(grp, filter) {
                var grouplists = Ext4.ComponentQuery.query('grouplistview');
                if (grouplists) {
        ...
        this.summaryLinkTask = new Ext4.util.DelayedTask(function(view) {
            var links = Ext4.DomQuery.select('a', view.getEl().id);
            for (var i=0; i < links.length; i++) {
                Ext4.get(links[i]).on('click', function(target, el) {
                    var nav = Ext4.get(el.id).getAttribute('nav');
                    if (Ext4.isString(nav) && nav.length > 0) {
                        this.navigate(nav);
                    }
...
//        v.on('beforeshow', this.beforeSummaryShow, this);
    ...
    onGroupChange : function(preview, grp) {
        if (Ext4.isArray(grp.data.filters)) {
    ...
    populateActiveFilter : function(rec) {
        ...
        if (summaryView) {
            var r;
            if (!rec) {
                r = Ext4.create('LABKEY.study.GroupCohort', {
    ...
    onSummarySelect : function(view) {
        ...
        if (state.getPrivateSelection('groupselection'))
        {
            ...
            if (filters.length > 0 && filters[0].groupLabel != 'Current Active Filters') {
                var grp = Ext4.create('Connector.model.FilterGroup', {
