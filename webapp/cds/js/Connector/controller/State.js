 ...
 */
Ext4.define('Connector.controller.State', {
    ...
    init : function() {
        ...
        this.state = Ext4.create('Ext.data.Store', {
...
//                this.setDetail(s.detail);
    ...
    updateView : function(viewname, viewstate, title, skipState) {
        ...
        if (Ext4.supports.History) {
            ...
            if (!this.POP_STATE && this.preventRedundantHistory && (this.lastAppState != appState)) {
                ...
                history.pushState({activeView : viewname}, 'Connector: ' + viewname, 'app.view?' + this.getURLParams() + '#' + appState);
            }
    ...
    updateState : function() {
        if (!this._updateState) {
            this._updateState = new Ext4.util.DelayedTask(function() {
                this.state.add({
                ...
                if (filters[s].data) {
                    if (filters[s].data.filters) {
                        ...
                        for (var f=0; f < filters[s].data.filters.length; f++) {
                            subfilters.push(Ext4.create('Connector.model.Filter', filters[s].data.filters[f].data));
                        }
                        filters[s].data.filters = subfilters;

                        newFilters.push(Ext4.create('Connector.model.FilterGroup', filters[s].data));
                    }
                    else {
                        newFilters.push(Ext4.create('Connector.model.Filter', filters[s].data));
                    }
                }
                else {
                    newFilters.push(Ext4.create('Connector.model.Filter', filters[s]));
                }
            }
    ...
    addPrivateSelection : function(selection, name) {
        ...
        var filters = [];
        if (Ext4.isArray(selection))
        {
            var newSelectors = [];
            for (var s=0; s < selection.length; s++) {

                if (!selection[s].$className)
                    newSelectors.push(Ext4.create('Connector.model.Filter', selection[s]));
