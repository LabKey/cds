 ...
 */
Ext4.define('Connector.controller.Explorer', {
    ...
    init : function() {
        ...
        this.clickTask = new Ext4.util.DelayedTask(function(view, rec, node) {
            if (view.btnclick)
            {
        ...
        this.hoverTask = new Ext4.util.DelayedTask(function(view, rec, add) {
            if (add) {
    ...
    createView : function(xtype, context) {
        ...
        if (xtype == 'singleaxis') {
            ...
            var v = Ext4.create('Connector.view.SingleAxisExplorer',{
                flex : 3,
                ui : 'custom',
                width : '100%',
                store : s,
    ...
    runSelectionAnimation : function(view, rec, node, callback) {
        ...
        var box   = Ext4.get(Ext4.DomQuery.select('.filterpanel')[1]).getBox(),
            child = Ext4.get(node).child('span.barlabel'),
            cbox  = child.getBox();
        ...
        if (callback) {
            var task = new Ext4.util.DelayedTask(callback, this, [view, rec, node]);
            task.delay(500);
        }
    },
