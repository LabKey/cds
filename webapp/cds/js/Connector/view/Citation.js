 ...
 */
Ext4.define('Connector.view.Citation', {
    ...
    createNorthPanel : function() {
        ...
        this.citationBtn = Ext4.create('Connector.button.RoundedButton', {
            margin  : '10 5 0 0',
            cls     : 'dark',
            text    : 'Citations',
            height  : 30,
    ...
    getCitationPanel : function() {
        ...
        var citationTpl = new Ext4.XTemplate(
...
//                                        '<a href="{link}" target="_blank">{title}</a>',
        ...
        citationTpl.renderDescription = function(val) {
            return Ext4.String.ellipsis(val, 250, true);
        }
        ...
        var dataView = Ext4.create('Ext.view.View', {
            ...
            listeners : {
                itemclick : function(view, rec, html, index, e, eOpts) {
                    if (Ext4.get(e.target).hasCls('cite-ref')) {
                        this.fireEvent('referencerequest', rec);
                    }
                    else if (Ext4.get(e.target).hasCls('cite-src')) {
        ...
        this.citationPanel = Ext4.create('Ext.Panel', {
            flex        : 1,
    ...
    getTitleDisplay : function() {
        ...
        if (!this.titleDisplay) {
            ...
            var bodyTpl = new Ext4.XTemplate(
                '<tpl>',
                    '<div class="nav-label">',
            ...
            bodyTpl.renderSrc = function() {
                if (me.src) {
                    if (Ext4.isString(me.src)) {
                        return Ext4.htmlEncode(me.src);
                    }
            ...
            this.titleDisplay = Ext4.create('Ext.view.View', {
                ui   : 'custom',
                flex : 1,
                tpl  : bodyTpl,
                store: this.store,
