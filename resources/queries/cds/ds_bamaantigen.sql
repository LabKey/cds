...
SELECT
  ...
  GROUP_CONCAT(DISTINCT p.panel_name, '|') AS panel_names
FROM cds.import_BAMAAntigen bama
    LEFT JOIN
    (SELECT ap.cds_ag_id, ap.cds_panel_id, apm.panel_name FROM cds.import_antigenPanelMeta apm INNER JOIN cds.import_antigenPanel ap ON ap.cds_panel_id = apm.cds_panel_id) p
    ON bama.cds_ag_id = p.cds_ag_id
